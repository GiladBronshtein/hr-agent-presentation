      ...
      <ContentLayout>
        <div style={{ paddingTop: 'clamp(1.5rem,3cqw,3rem)' }} className="w-full max-w-7xl space-y-8">
          <div className="text-center">
            <p className="text-white/60 text-3xl uppercase tracking-widest mb-2">הגדרה</p>
          ...
          <div className="grid grid-cols-3 md:grid-cols-7 gap-5">
            {COMPONENTS.map((comp, i) => (
              <div key={i} className="flex flex-col items-center gap-2 p-3 rounded-xl text-center"
                ...
                <span className="text-white/80 text-3xl font-bold">{comp.label}</span>
                <span className="text-white/60 text-3xl leading-tight">{comp.desc}</span>
              </div>
            ))}
          </div>
